...
const TourList = () => {
  ...
  const fetchTours = async () => {
    try {
      ...
      const response = await fetch(
        `${baseUrl}/tours/userTours/${userDetails._id}`
      );
      if (!response.ok) {
  ...
  const cancelTour = async (tourId) => {
    const response = await fetch(`${baseUrl}/tours/cancelTour/${tourId}`);
    if (!response.ok) {
      throw new Error("Network response was not ok");
    }
    const data = await response.json();
        ...
        {toursData?.map((tour, index) => (
          <div key={index} className="shadow-lg rounded-lg p-4">
            ...
            <div className="flex justify-center flex-col">
              ...
              <p
                className={`text-lg font-bold mb-4 text-center w-full ${
                  tour.status === "scheduled"
                    ? "text-blue-500"
              ...
              >
                Status : {tour.status} by{" "}
                {tour?.cancelledBy === "buyer"
                  ? "you"
                  : tour?.cancelledBy || ""}
              </p>
              ...
              {new Date(tour?.date).toLocaleDateString() ===
                ...
                tour.status === "scheduled" && (
                  <button
                    className="px-4 py-2 bg-yellow-500 text-white rounded hover:bg-blue-600  w-full mb-2"
              ...
              {tour.status === "completed" && !tour?.isRated && (
                <button
                  className="px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600  w-full mb-2"
